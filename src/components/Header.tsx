...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border/80 bg-surface/90 backdrop-blur-md">
      <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between gap-4 px-4 py-2.5 sm:px-6 lg:px-8">
        <Link href="/" className="shrink-0 rounded-md" aria-label="Фармакея — на главную">
          <Logo />
        </Link>
      ...
      {mobileOpen ? (
        <div className="fixed inset-x-0 top-[72px] bottom-0 z-40 overflow-y-auto bg-surface px-4 pb-10 pt-4 lg:hidden">
          <nav aria-label="Мобильная навигация" className="flex flex-col gap-1">
            {primaryNav.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="flex flex-col rounded-xl px-4 py-3.5 transition-colors hover:bg-primary-50 active:bg-primary-100"
              >
          ...
          <div className="mt-6 flex flex-col gap-3 border-t border-border pt-6">
            <Button href="/apteki" size="lg" className="w-full">
              Найти аптеку
            </Button>
            <Button href="/veterinariya" variant="secondary" size="lg" className="w-full">
              Ветеринарные клиники
            </Button>
            <Button href="/kontakty" variant="outline" size="lg" className="w-full">
              Контакты
            </Button>
          </div>
        </div>
